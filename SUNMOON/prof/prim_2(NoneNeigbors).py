...
class Graph:
  ...
  def update(self, node):
    ...
    for neighbor_index in range(0, len(nodes)):
      weight =  matrix[node_index][neighbor_index]
      if weight > 0 and not nodes[neighbor_index].get_is_done():
        nodes[neighbor_index].set_key(weight, node)
